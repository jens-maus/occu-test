...
const regaOutput = true; // Set to true to show stdout/stderr of ReGaHss process
...
function startRPC() {
    rpcClient = binrpc.createClient({host: '127.0.0.1', port: '1999', reconnectTimeout: 0});
}
...
function startRega(flavor, faketime, nocopy = false) {
    ...
    if (faketime) {
        ...
        procs.rega = cp.spawn('unbuffer', ['/bin/ReGaHss' + flavor, '-c', '-l', '0', '-f', '/etc/rega.conf'], {LD_LIBRARY_PATH: '/occu/X86_32_Debian_Wheezy/packages-eQ-3/WebUI/lib/'});
    }
